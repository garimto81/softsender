...
### v11.3.9 (2025-01-16)
**🔍 테이블 누락 문제 진단 시스템 추가**
- ✅ **CacheService 크기 제한 감지**: 100KB 초과 시 캐싱 비활성화
- ✅ **데이터 크기 측정**: 서버/클라이언트 양측에서 JSON 크기 로깅
- ✅ **테이블별 통계**: 서버에서 테이블 범위, 개수, 목록 출력
- ✅ **클라이언트 검증**: 수신된 테이블 번호 범위 및 행 수 확인
- 🎯 **목적**: 23-25번 테이블 누락 원인 파악 (캐시 크기 vs 데이터 손실)

### v11.3.7 (2025-01-16)
**🚀 성능 최적화: SC 번호 캐싱**
- ✅ **CacheService 도입**: SC 번호 캐싱으로 10초 → 1초 (90% 개선)
- ✅ **getCachedSCNumber()**: 5분 TTL 캐시, 캐시 미스 시에만 F열 읽기
- ✅ **성능 측정 도구**: testCachedSCNumber() 함수 추가 (3회 연속 호출 벤치마크)
- ✅ **디버깅 개선**: 전체 소요시간 측정 로그 추가
- 🎯 **성능**: SpreadsheetApp.openById() 병목 해결 (~10s → ~1ms)
