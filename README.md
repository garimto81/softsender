...
### v11.5.0 (2025-01-16)
**🔒 SC 번호 2단계 예약 패턴 (완전한 동시성 제어)**
- ✅ **2단계 예약 시스템**:
  - 1단계: `reserveSCNumber()` - Lock 내에서 F열에 `SC###_RESERVED` 마커 작성
  - 2단계: `updateVirtual()` - 최종 파일명으로 예약 마커 덮어쓰기
- ✅ **Race Condition 완전 해결**: Lock 해제 전 F열에 예약 완료
- ✅ **동시 요청 안전성**: 예약 마커가 F열에 즉시 기록되어 중복 번호 0%
- ✅ **테스트 함수 개선**: `testReserveSCNumber()` - 예약 마커 검증 추가
- 🎯 **신뢰성**: 여러 사용자 동시 요청 시에도 SC 번호 순차 증가 보장
